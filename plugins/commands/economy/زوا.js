...
export const config = {
  name: "زوجني",
  version: "0.0.2-xaviabot-genderfix",
  description: "زواج عشوائي بينك وبين أحد أعضاء المجموعة (حسب النوع)",
  cooldown: 15
};
...
// 🧠 دالة جلب النوع (ذكر / أنثى)
async function getGender(userID) {
  const info = await global.controllers.Users.getInfo(userID);
...
export async function onCall({ message, api}) {
  try {
    ...
    const senderGender = await getGender(senderID);
    const targetGender = senderGender === "male"? "female": "male";

    const filteredList = [];
    for (const id of listUserID) {
      const gender = await getGender(id);
      if (gender === targetGender) filteredList.push(id);
}

    if (filteredList.length === 0) {
      return message.reply("ما لقيت شريك مناسب حسب النوع المطلوب 😢");
}

    const matchID = filteredList[Math.floor(Math.random() * filteredList.length)];
